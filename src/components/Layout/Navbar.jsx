        ...
        <div className=' d-flex shadow align-items-center justify-content-between px-5 py-3 w-100' style={{backgroundColor:'#1B1744', zIndex:'999'}}>
            ...
            {userData.isLogged && (
                <div className='d-flex gap-5 align-items-center '>
                    <Link to='/perfil' className='text-decoration-none'><span className='text-white fw-bold' >{userData.data.nombre}</span></Link>
                    <button className='btn btn-light' onClick={() => setUserData({isLogged:false, data:{}})}>Cerrar sesión</button>
                </div>
            )}
        </div>
    )
